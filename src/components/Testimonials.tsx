...
export default function Testimonials() {
  ...
  return <section id="testimonials" ref={sectionRef} className="py-32 px-6 md:px-12 bg-secondary/30 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className={`relative transition-all duration-700 delay-300 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`} style={{
        transform: `translate(${normalizedX * -10}px, ${normalizedY * -10}px)`,
        transition: "transform 0.3s ease-out"
      }}>
          <div className="py-16 relative">
            {/* Large Quote Mark */}
            <div className="absolute top-0 left-0 text-[200px] leading-none text-primary/10 font-black pointer-events-none select-none" style={{
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-16 items-center">
              <div className="md:col-span-4 order-2 md:order-1">
                <div className="relative h-[350px] w-full">
                  {testimonials.map((testimonial, index) => <div key={testimonial.id} className={`absolute inset-0 transition-all duration-700 ${activeIndex === index ? "opacity-100 scale-100" : "opacity-0 scale-90 pointer-events-none"}`}>
                      <div className="h-full w-full flex flex-col items-center justify-center relative">
                        <div className="w-64 h-64 rounded-full overflow-hidden border-8 border-background relative mb-6" style={{
                      transform: activeIndex === index ? `translate(${normalizedX * 15}px, ${normalizedY * 15}px)` : "none",
                      transition: "transform 0.3s ease-out"
                    }}>
              ...
              <div className="md:col-span-8 order-1 md:order-2">
                <div className="relative min-h-[220px] flex items-center">
                  {testimonials.map((testimonial, index) => <div key={testimonial.id} className={`w-full absolute transition-all duration-700 ${activeIndex === index ? "opacity-100 translate-x-0" : direction === 'right' ? "opacity-0 -translate-x-20 pointer-events-none" : direction === 'left' ? "opacity-0 translate-x-20 pointer-events-none" : "opacity-0 translate-x-0 pointer-events-none"}`}>
                ...
                <div className="mt-12 flex items-center justify-between">
                  ...
                  }} className={`w-3 h-3 rounded-full transition-all duration-300 ${index === activeIndex ? "w-10 bg-primary" : "bg-primary/30"}`} aria-label={`Go to testimonial ${index + 1}`} />)}
                  </div>
