...
export default function Services() {
  ...
  return <section id="services" ref={sectionRef} className="py-24 relative overflow-hidden" data-cursor="design">
      ...
      <div className="relative z-10">
        ...
        <div className="container px-4 md:px-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {services.map(service => <div key={service.id} className={`rounded-xl p-8 md:p-10 relative overflow-hidden transition-all duration-500 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-20"} ${hoverService === service.id ? 'scale-[1.02]' : 'scale-100'}`} onMouseEnter={() => setHoverService(service.id)} onMouseLeave={() => setHoverService(null)} style={{
            ...
            boxShadow: hoverService === service.id ? '0 10px 40px rgba(0, 0, 0, 0.1)' : 'none'
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-accent-foreground/5 opacity-50 transition-opacity duration-500" style={{
